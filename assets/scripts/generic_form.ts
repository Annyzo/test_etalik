...
const getDeleteButtonCTA = (): HTMLButtonElement => {
  ...
  const classes = ['btn', 'btn-danger', 'btn-sm', 'mx-2', 'suppresion-button']
...
const addField = () => {
  if (!checkIsSomeFieldsAreNotFilled()) {
    // Crée un nouvel élément div pour le champ
    const divSection = document.createElement('div');
    divSection.classList.add('field-row');
    divSection.innerHTML = getSanitizedTemplate();
    ...
    // Ajoute une ligne de séparation horizontale après chaque ajout
    const hr = document.createElement('hr');
    container?.appendChild(hr);

    // Gère la sélection du type de champ
    divSection.querySelector('.field-type-selection')?.addEventListener('change', handleFieldTypeSelection);
  } else {
    toastError({
...
// Assurez-vous que la fonction `getSanitizedTemplate` génère correctement le HTML pour le nouveau champ
